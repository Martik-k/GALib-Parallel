...
def update(gen):
    ...
    fig.suptitle(f"Standard GA Evolution: {name}\nGeneration: {gen} | Bounds: [{lb}, {ub}]", fontsize=16)
...
photo_path = os.path.join(IMG_DIR, f'plot_standard_{name.lower()}.png')
print(f"Static plot saved for report: {photo_path}")

mp4_path = os.path.join(ANI_DIR, f'standard_{name.lower()}.mp4')
gif_path = os.path.join(ANI_DIR, f'standard_{name.lower()}.gif')

print(f"Saving animations for Standard GA {name} to {ANI_DIR}...")
