...
\begin{document}
    ...
    \begin{abstract}
        While Genetic Algorithms (GAs) are powerful optimization tools, their high computational cost limits scalability. This paper details the development of a genetic algorithm library that includes shared-memory, distributed-memory, and accelerator-based parallelization strategies to solve this problem. We describe an implemented multi-threaded Generational GA, a CUDA-accelerated GA, an MPI-based Island Model, a oneTBB-parallelized Cellular GA, and an OpenMP-optimized Differential Evolution algorithm. The performance of this library is evaluated using various 3D mathematical surfaces and computationally intensive benchmark functions.
    \end{abstract}
    ...
    We compared the CUDA backend against the multi-threaded OpenMP implementation across several benchmark functions. Figure~\ref{fig:cuda_speedup_pop} shows that speedup grows with population size: fixed overheads (kernel launch, memory transfers, setup) are amortized, GPU occupancy improves, and arithmetic intensity increases. For small populations, the overhead dominates and GPU benefits are limited.

    Figure~\ref{fig:cuda_best_fitness} compares final best fitness (lower is better). Differences across CPU and CUDA runs reflect stochasticity (random seeds, selection noise) and parallel execution order. CUDA primarily reduces time-to-solution; it does not guarantee better solution quality. It helps when larger populations can be evaluated faster, enabling more exploration in the same wall time. It does not help when the GPU path still relies on host-side evaluation or when population sizes are too small to saturate the device.

    \begin{figure}[H]
        \centering
        \includegraphics[width=\columnwidth]{media/speedup_vs_popsize_cuda.png}
        \caption{CUDA speedup vs population size across benchmark functions.}
        \label{fig:cuda_speedup_pop}
    \end{figure}

    \begin{figure}[H]
        \centering
        \includegraphics[width=\columnwidth]{media/fitness_quality_cuda.png}
        \caption{Final best fitness comparison for CPU vs CUDA (lower is better).}
        \label{fig:cuda_best_fitness}
    \end{figure}
    ...
    \subsection{Benchmark Suite Performance}
    The library includes a comprehensive benchmark suite for evaluating algorithm performance across different parallelization strategies. Time benchmarks measure execution speedup with increasing thread counts on the Rastrigin function, quality benchmarks assess convergence to optimal solutions, and convergence plots visualize fitness improvement over generations. Benchmarks were conducted on an HP 250 G8 Notebook PC with Intel Core i5-1135G7 (4 cores, 8 threads).

    \begin{figure}[H]
        \centering
        \includegraphics[width=\columnwidth]{media/cellular_time_benchmark.jpg}
        \caption{Performance benchmarks for the Cellular Genetic Algorithm on the Rastrigin function, showing execution time reduction and speedup as thread count increases from 1 to 4.}
        \label{fig:cellular_time}
    \end{figure}

    \begin{figure}[H]
        \centering
        \includegraphics[width=\columnwidth]{media/de_time_benchmark.jpg}
        \caption{Performance benchmarks for Differential Evolution on the Rastrigin function, showing execution time reduction and speedup as thread count increases from 1 to 4.}
        \label{fig:de_time}
    \end{figure}

    \begin{figure}[H]
        \centering
        \includegraphics[width=\columnwidth]{media/standard_time_benchmark.jpg}
        \caption{Performance benchmarks for the Standard Genetic Algorithm on the Rastrigin function, showing execution time reduction and speedup as thread count increases from 1 to 4.}
        \label{fig:standard_time}
    \end{figure}
...
\end{verbatim}
    ...
    \section{Conclusion}
    This paper presented GALib-Parallel, a comprehensive C++20 genetic algorithm library that addresses computational scalability challenges through multi-tiered parallelization. The library integrates OpenMP, MPI, CUDA, and oneTBB to deliver multiple evolutionary models---Standard GA, Island Model, Cellular GA, and Differential Evolution---within a unified, extensible framework. Empirical validation demonstrates substantial performance gains, with CUDA acceleration achieving speedups of 3.70$\times$ to 12.04$\times$ on compute-intensive workloads and the hybrid Island Model delivering 2.32$\times$ speedup across an 18-core heterogeneous cluster. By automating parallel implementation complexity through a template-driven design and configuration-driven API, GALib-Parallel enables practitioners to leverage modern computing resources for previously intractable optimization problems. The library's accessibility and extensibility position it as a robust, production-ready platform for evolutionary computation at scale.
