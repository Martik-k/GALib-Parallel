...
\begin{document}
    ...
    Every possible solution in a GA is represented as a sequence of parameters called a or \textit{genotype}. How this solution actually looks or works in the problem is the \textit{phenotype}. Each solution is evaluated using a \textit{fitness function}, $F(x)$, which gives a score based on how well it solves the problem. The main goal of the algorithm is to find a genotype with the highest (or lowest) fitness score.
    ...
    \subsection{Schema Theory and Premature Convergence}
        The mathematical foundation of Genetic Algorithms is based on \textit{schemas}---templates that describe a subset of chromosomes sharing specific genes. 
