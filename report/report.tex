...
\begin{document}
    ...
    To solve this, different parallelization methods can be used, depending on the hardware and the specific algorithm. This project aims to develop a fast C++ library that includes four different algorithms: a classic Generational GA, an Island Model GA, a Cellular GA, and Differential Evolution (DE). Using tools like OpenMP, OpenMPI, oneTBB, and CUDA, we will adapt these algorithms to the best hardware options (shared memory, distributed memory, and GPUs). 
    ...
    \subsection{CUDA Implementation of Generational Genetic Algorithm}
    ...
    Experiments were run on an NVIDIA GeForce RTX 4050 Laptop GPU (6 GB VRAM) using the HeavyTrig benchmark. Figure~\ref{fig:cuda_ga_speedup} summarizes measured speedups against the OpenMP CPU backend.

    \begin{figure}[t]
        \centering
        \includegraphics[width=\columnwidth]{figures/cuda_ga_speedup.png}
        \caption{CUDA speedup for Generational GA on HeavyTrig benchmark.}
        \label{fig:cuda_ga_speedup}
    \end{figure}

    The measured range shows consistent GPU advantage for compute-heavy workloads, with speedups from 3.70$\times$ to 12.04$\times$.
    ...
    In the implemented Cellular Genetic Algorithm, the population is represented as a two-dimensional grid. Each cell stores one individual, and reproduction is performed locally rather than across the whole population. For every cell, one parent is the current individual itself, while the second parent is selected from the Von Neumann neighborhood consisting of the four adjacent cells (up, down, left, and right). The grid uses cyclic boundaries, so edge cells remain connected to the opposite side of the population.

    Parent selection is performed by choosing the best neighboring individual according to fitness. The selected pair then undergoes crossover with probability $p_c$ and mutation with probability $p_m$. Two offspring are produced and evaluated immediately. If local elitism is enabled, the algorithm keeps the best individual among the current parent and the two offspring; otherwise, only the better offspring survives. The next generation is formed in a separate grid and replaces the previous one synchronously after all cells have been processed.

    This local interaction pattern helps preserve diversity and slows down premature convergence compared to fully panmictic models. The implementation is parallelized with oneTBB: both population evaluation and cell-wise generation updates are executed with \texttt{tbb::parallel\_for}, since each cell can be evolved independently from the previous generation state.
